...
class VesselRegistry:
    ...
    def __init__(self, cache_file: Optional[str] = None) -> None:
        ...
        Logger.info("Vessel registry initialized with %d vessel names", len(self.vessel_names))
    ...
    def _load_cache(self) -> None:
        ...
        if os.path.exists(self.cache_file):
            try:
                ...
                Logger.info("Loaded %d vessel names from cache file", len(self.vessel_names))
    ...
    def update_vessel_static(self, data: Dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            # Save to cache file every 5 new entries, but only if cache file is specified
...
class AISStreamClient:
    ...
    async def _handle_message(self, message_json: str) -> None:
        ...
        try:
            ...
            if not ais_message:
                self._logger.debug("Could not extract message content for type: %s", message_type)
                return
            ...
            if "MetaData" in message and message["MetaData"]:
                ...
                if mmsi:
                    ...
                    ship_name = (metadata.get("ShipName", "") or 
                                ...
                                metadata.get("VesselName", "")).strip()
                    
                    if ship_name:
                        self._logger.info("Found ship name in MetaData: MMSI=%s, Name=%s", mmsi, ship_name)
                    ...
                    if callsign:
                        self._logger.debug("Found callsign in MetaData: MMSI=%s, CallSign=%s", mmsi, callsign)
                
            # For most message types, we need to transform to a format compatible with our existing code
            transformed_message = self._transform_aisstream_message(ais_message, message_type)
    ...
    def _transform_aisstream_message(self, ais_message: Dict[str, Any], message_type: str) -> Optional[Dict[str, Any]]:
        ...
        if "Name" in ais_message and ais_message["Name"]:
            ...
            if vessel_name:
                ...
                self.registry.update_vessel_static({
                    "mmsi": mmsi,
                    "name": vessel_name,
                    "source": f"{message_type}"
                })
                self._logger.debug("Found vessel name in %s: MMSI=%s, Name=%s", 
                                 message_type, mmsi, vessel_name)
        
        # Handle ShipName as a separate field if present (to match core application's expectations)
        if "ShipName" in ais_message and ais_message["ShipName"]:
            ...
            if shipname:
                ...
                if not "name" in result or shipname != result["name"]:
                    result["shipname"] = shipname
                    self._logger.debug("Found different shipname: MMSI=%s, Shipname=%s", 
                                     mmsi, shipname)
        ...
        if message_type == "PositionReport":
            ...
            # Store Course Over Ground - standardize on lowercase "cog" as that's what our cot generator expects
            ...
            # Store Speed Over Ground - standardize on uppercase SOG as that's what our cot generator expects
            ...
            if "TrueHeading" in ais_message:
                ...
                if true_heading == 511:
                    ...
                    if "Cog" in ais_message and ais_message["Cog"] is not None:
                        ...
                        self._logger.debug("Using COG as heading since TrueHeading not available")
                else:
                    result["heading"] = float(true_heading)
                    self._logger.debug("TrueHeading from PositionReport: %s", true_heading)
            ...
            self._logger.debug("ShipStaticData raw: MMSI=%s, Name='%s', CallSign='%s', Eta=%s", 
            ...
            # Pre-filter messages without position data
            if not lat or not lon:
                self._logger.debug("Skipping StandardClassBPositionReport with missing lat/lon")
                return None
            ...
            if "TrueHeading" in ais_message:
                ...
                if true_heading == 511:
                    ...
                    result["heading"] = float(true_heading)
                    self._logger.debug("TrueHeading from StandardClassBPositionReport: %s", true_heading)
            ...
            # Pre-filter messages without position data
            if not lat or not lon:
                self._logger.debug("Skipping ExtendedClassBPositionReport with missing lat/lon")
                return None
            ...
            if "TrueHeading" in ais_message:
                ...
                if true_heading == 511:
                    ...
                    result["heading"] = float(true_heading)
                    self._logger.debug("TrueHeading from ExtendedClassBPositionReport: %s", true_heading)
    ...
    async def _process_message(self, msg: Dict[str, Any]) -> None:
        ...
        if (
            self.known_craft_db
            and not known_craft
            and not self.config.getboolean("INCLUDE_ALL_CRAFT", False)
        ):
            return
    ...
    def _normalize_ais_data(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "heading" in normalized:
            try:
                ...
                if heading_val == 511 or heading_val < 0 or heading_val > 360:
                    # If heading is invalid but we have COG (Course Over Ground), use that instead
        ...
        # NOTE: The ais_to_cot function expects speed in 0.1-knot units and converts with:
        # sog = float(sog) * 0.1 / 1.944
        # But AISStream.io provides speeds directly in knots, so we need to multiply by 10
        # to compensate for the 0.1 multiplication in ais_to_cot
        if "SOG" in normalized:
            try:
                # Multiply by 10 to convert from knots to 0.1-knot units expected by ais_to_cot
                sog_knots = float(normalized["SOG"])
                sog_adjusted = sog_knots * 10
                normalized["SOG"] = sog_adjusted
                self._logger.debug("Adjusted SOG from %s knots to %s (0.1-knot units)", 
                                  sog_knots, sog_adjusted)
            except (ValueError, TypeError):
        ...
        for coord in ["lat", "lon"]:
            if coord in normalized:
                try:
                    ...
                    # If coordinate can't be converted to float, this message will be skipped later
